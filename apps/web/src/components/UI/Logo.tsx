...
const Logo: React.FC<LogoProps> = ({ className = '', size = 'md', hideText = false }) => {
  ...
  return (
    <div className={`flex items-center ${currentSize.gap} ${className}`}>
      <div className={`${currentSize.icon} bg-indigo-600 rounded-xl flex items-center justify-center shadow-[0_0_15px_rgba(99,102,241,0.4)] border border-indigo-400/20`}>
        <div className="w-[45%] h-[45%] border-2 border-white rounded-md"></div>
      </div>
      {!hideText && (
        <span className={`text-white font-geist font-black tracking-tighter ${currentSize.text}`}>
          Buildora
        </span>
      )}
    </div>
