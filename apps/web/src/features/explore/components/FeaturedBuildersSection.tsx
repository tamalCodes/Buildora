...
const FeaturedBuildersSection: React.FC<FeaturedBuildersSectionProps> = ({
  ...
  <section className="space-y-10">
    <div className="space-y-6 scroll-mt-24" id="explore-builders">
      ...
      <div className="glass-card rounded-[2.5rem] border border-(--border-default) p-4 sm:p-5 lg:p-6">
        <div className="grid grid-cols-1 gap-5 lg:gap-6 2xl:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)]">
          <article className="relative min-h-80 overflow-hidden rounded-4xl border border-(--border-default) bg-(--bg-elevated) shadow-[0_24px_70px_rgba(15,23,42,0.14)] sm:min-h-75 lg:min-h-85">
            <img
              src={BUILDER_SPOTLIGHT.imageUrl}
              alt={BUILDER_SPOTLIGHT.name}
              className="absolute inset-0 h-full w-full object-cover"
            />
            <div className="absolute inset-0 bg-(--spotlight-image-overlay)" />
            ...
            <div className="relative z-10 hidden h-full flex-col justify-between p-4 sm:flex sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.3em] text-(--spotlight-text-strong)">
                    Builder spotlight
                  </p>
                  <p className="mt-2 text-sm text-(--spotlight-text-muted)">
                    {BUILDER_SPOTLIGHT.location}
                  </p>
                </div>
              </div>

              <div className="w-full rounded-[1.4rem] border border-white/10 bg-(--spotlight-content-bg) p-4 backdrop-blur-sm sm:max-w-[82%] sm:rounded-3xl sm:p-5">
                <div className="space-y-3 sm:space-y-4">
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="text-base font-bold text-(--spotlight-text-strong) sm:text-lg">
                          {BUILDER_SPOTLIGHT.name}
                        </p>
                        <p className="text-xs text-(--spotlight-text-body) sm:text-sm">
                          {BUILDER_SPOTLIGHT.role}
                        </p>
                      </div>
                    </div>
                    <h4 className="max-w-[13ch] text-[2.05rem] font-geist font-black leading-[0.95] tracking-tight text-(--spotlight-text-strong) sm:max-w-[14ch] sm:text-5xl">
                      {BUILDER_SPOTLIGHT.title}
                    </h4>
                    <p className="max-w-lg text-sm font-semibold text-(--spotlight-text-strong) sm:text-base">
                      {BUILDER_SPOTLIGHT.headline}
                    </p>
                    <p className="hidden max-w-lg text-sm leading-6 text-(--spotlight-text-body) sm:block">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="hidden text-sm text-(--spotlight-text-body) sm:block">
                        {BUILDER_SPOTLIGHT.currentProject}
                      </p>
                      <p className="text-xs text-(--spotlight-text-muted) sm:mt-1">
          ...
          <div className="hidden space-y-4 sm:block">
            {FEATURED_BUILDERS.slice(0, 2).map((builder) => (
              <article
                key={builder.id}
                className="rounded-[1.75rem] border border-(--border-default) bg-(--bg-surface) p-4 shadow-[0_14px_40px_rgba(15,23,42,0.06)] transition hover:-translate-y-0.5 hover:border-(--accent-border)"
              >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-3 text-lg font-geist font-black leading-tight text-(--text-heading)">
                          {builder.headline}
                        </p>
                      </div>
                    </div>
