      ...
      <div className="space-y-4">
        {detail.schedule.map((item, index) => (
          <div
            ...
            className="rounded-[1.5rem] border border-[var(--border-default)] bg-[var(--bg-surface)] px-6 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4"
          >
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-[var(--text-tertiary)]">
                {item.time}
              </p>
              <p className="text-lg font-bold text-[var(--text-heading)] mt-2">{item.title}</p>
              <p className="text-sm text-[var(--text-secondary)] mt-2">
                {item.description}
              </p>
            </div>
            <div className="text-xs font-black uppercase tracking-[0.3em] text-[var(--accent-text)]">
              Step {index + 1}
            </div>
          </div>
        ))}
