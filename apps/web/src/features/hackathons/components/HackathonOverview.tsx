...
const HackathonOverview: React.FC<HackathonOverviewProps> = ({ detail }) => {
  return (
    <section id="overview" className="space-y-8">
      ...
      <div className="rounded-[2rem] border border-[var(--border-default)] bg-[var(--bg-surface)] p-8 space-y-6">
            ...
            <ul className="space-y-2 text-sm text-[var(--text-secondary)]">
              {detail.perks.map((perk) => (
                <li key={perk} className="flex items-start gap-2">
                  <span className="mt-1 h-2 w-2 rounded-full bg-[var(--accent-border-active)]"></span>
                  {perk}
                </li>
              ))}
            </ul>
          ...
          <ul className="space-y-2 text-sm text-[var(--text-secondary)] mt-4">
            {detail.rules.map((rule) => (
              <li key={rule} className="flex items-start gap-2">
                <span className="mt-1 h-2 w-2 rounded-full bg-[var(--text-muted)]"></span>
                {rule}
              </li>
            ))}
          </ul>
