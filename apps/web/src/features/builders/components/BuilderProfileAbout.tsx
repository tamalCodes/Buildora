...
const BuilderProfileAbout: React.FC<BuilderProfileAboutProps> = ({
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-[2rem] border border-[var(--border-default)] bg-[var(--bg-surface)] p-6 sm:p-8 space-y-6">
        <div>
      ...
      <div className="rounded-[2rem] border border-[var(--border-default)] bg-[var(--bg-surface)] p-6 sm:p-8">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--text-tertiary)]">
          Core strengths
        </p>
        <ul className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-[var(--text-primary)]">
          {strengths.map((item) => (
            <li
              key={item}
              className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-input)] px-4 py-3"
            >
              {item}
            </li>
          ))}
        ...
        <div className="rounded-[2rem] border border-[var(--border-default)] bg-[var(--bg-surface)] p-6 sm:p-8">
          ...
          <div className="mt-5 space-y-4">
            {profile.experience.map((item) => (
              <article
                key={item.id}
                className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-input)] px-4 py-4"
              >
